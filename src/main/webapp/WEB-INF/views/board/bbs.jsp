          ...
          <tr>
          	<td colspan=7>
          		<c:if test="${pagevo.prev }">
						<a href="board?page=${pagevo.startPage -1}&searchKeyword=${pagevo.searchKeyword}">[이전페이지그룹] </a>
					</c:if>
				<!--  forEach   리스틀 순회,  아래처럼 시작과 끝을 지정하여 반복하는 사용것. -->			
					<c:forEach begin="${pagevo.startPage}" end="${pagevo.endPage }" var="idx">
						<c:if test="${pagevo.page == idx}">[</c:if>
						<a href="board?page=${idx}&searchKeyword=${pagevo.searchKeyword}">${idx}</a>	
						<c:if test="${pagevo.page == idx}">]</c:if>				
					</c:forEach>	
					<c:if test="${pagevo.next }">
						<a href="board?page=${pagevo.endPage + 1}&searchKeyword=${pagevo.searchKeyword}"> [다음페이지그룹]</a>
					</c:if>	
          	</td>
          </tr>
          </tbody>
